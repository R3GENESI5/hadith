...
| 👤 | **Narrator database** | 117,893 narrators with 178,859 name variants, jarh wa ta'dil from 15 classical texts (152,000+ entries) |
...
What Wensinck's team took decades to compile for 9 books, Itqan computes for 18. But more importantly, **Itqan builds the bridge that never existed** — connecting Fuad Abd al-Baqi's Quran concordance to Wensinck's hadith concordance through 1,590 shared Arabic roots, generating 1,528,346 verified cross-references. And what required a scholar to open four volumes of rijal, Itqan consolidates from 15 classical texts into one searchable database of 117,893 narrator profiles.
...
Everything is open JSON files. A researcher can load `quran_hadith_bridge.json` and query all 1,528,346 root connections programmatically. A developer can build a new app on `narrator_unified.json` and its 117,893 profiles. No API keys, no rate limits, no backend. The live web app is fully static and works offline.
...
| **Unified rijal database** | 117,893 narrator profiles with grades, kunya, jarh wa ta'dil merged from 3 sources | 178,859 name variants, 31,822 classical source cross-refs |
...
`app/rijal.html` — searchable browser for **117,893 narrator biographies** with:
...
- **178,859 name variants** — every spelling a narrator appears under across manuscripts
...
Grades are matched from a unified database of **117,893 narrators** (178,859 name variants) compiled from three sources:
...
graph TD
    subgraph ITQAN["ITQAN (static, offline-capable)"]
        ...
        RJ["👤 Rijal Profiles<br/>117,893 narrators · jarh wa ta'dil"]
...
graph LR
    ...
    ARSANAD["📋 AR-Sanad 280K"] --> UNIFIED["narrator_unified.json<br/>117,893 profiles · 118 MB"]
...
│   ├── rijal.html                Narrator profiles browser (117,893 narrators, jarh wa ta'dil)
...
The `narrated_from` and `narrated_to` fields contain **narrator ID cross-references** — a complete teacher→student network across 117,893 narrators. This would allow:
...
| 10 | v1.4: Rijal page + AR-Sanad 117,893 narrators + hatemben jarh wa ta'dil | ✅ Complete |
| 11 | Kunya→real name tooltips (32 entries) + isnad parsing fixes | ✅ Complete |
| 12 | v1.5: Parse 8 classical rijal texts from OpenITI (152,000+ entries → 117,893 profiles) | ✅ Complete |
...
| **v1.5** | Apr 9 | 8 classical rijal texts parsed from OpenITI | 117,893 narrators, 83k entries |
...
### Data sources
- **AR-Sanad 280K** (somaia02) — 117,893 narrators with name variants, grades, and teacher-student links
