...
| 👤 | **Narrator database** | 106,207 narrators with 148,666 name variants, jarh wa ta'dil from 22 classical texts (152,000+ entries) |
...
Everything is open JSON files. A researcher can load `quran_hadith_bridge.json` and query all 1,528,346 root connections programmatically. A developer can build a new app on `narrator_unified.json` and its 106,207 profiles. No API keys, no rate limits, no backend. The live web app is fully static and works offline.
...
| **Unified rijal database** | 106,207 narrator profiles with grades, kunya, jarh wa ta'dil merged from 3 sources | 148,666 name variants, 31,822 classical source cross-refs |
...
`app/rijal.html` — searchable browser for **106,207 narrator biographies** with:
...
- **148,666 name variants** — every spelling a narrator appears under across manuscripts
...
Grades are matched from a unified database of **106,207 narrators** (148,666 name variants) compiled from three sources:
...
graph TD
    subgraph ITQAN["ITQAN (static, offline-capable)"]
        ...
        RJ["👤 Rijal Profiles<br/>106,207 narrators · jarh wa ta'dil"]
...
graph LR
    ...
    ARSANAD["📋 AR-Sanad 280K"] --> UNIFIED["narrator_unified.json<br/>106,207 profiles · 118 MB"]
...
│   ├── rijal.html                Narrator profiles browser (106,207 narrators, jarh wa ta'dil)
...
│       ├── narrator_unified.json      118 MB   — 106,207 rijal profiles (gitignored, regenerable)
...
The `narrated_from` and `narrated_to` fields contain **narrator ID cross-references** — a complete teacher→student network across 106,207 narrators. This would allow:
...
| 10 | v1.4: Rijal page + AR-Sanad 106,207 narrators + hatemben jarh wa ta'dil | ✅ Complete |
| 11 | Kunya→real name tooltips (32 entries) + isnad parsing fixes | ✅ Complete |
| 12 | v1.5: Parse 8 classical rijal texts from OpenITI (152,000+ entries → 106,207 profiles) | ✅ Complete |
...
| **v1.5** | Apr 9 | 8 classical rijal texts parsed from OpenITI | 106,207 narrators, 83k entries |
...
### Data sources
- **AR-Sanad 280K** (somaia02) — 106,207 narrators with name variants, grades, and teacher-student links
